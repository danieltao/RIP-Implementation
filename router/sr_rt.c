...
void *sr_rip_timeout(void *sr_ptr) {
    ...
    while (1) {
		...
		while(rt_walker_prev->updated_time + expiretime < now){
			/* if it is one of its ports*/
			if(rt_walker_prev -> metric==0){
				if(!sr_obtain_interface_status(sr, rt_walker_prev->interface)){/*it is closed*/
					rt_walker_prev->metric = 17;
					break;
				}
				rt_walker_prev = rt_walker_prev -> next;
			}else if (rt_walker_prev ->metric ==17){
				break;
			}
		...
		while (if_iter){
			...
			while(rt_iter){
				if((if_iter->ip & rt_iter->mask.s_addr) == (rt_iter->dest.s_addr & rt_iter->mask.s_addr) && sr_obtain_interface_status(sr, if_iter->name)!=0){
					...
					printf("Changed %s metric back to 0! \n", rt_iter->interface);
				}
		...
		while(rt_walker){
			...
			if (rt_walker->updated_time + expiretime < now && rt_walker->metric != 0 && rt_walker->metric !=17){
				printf("remove interface %s with metrics %d\n", rt_walker->interface, rt_walker -> metric);
				fflush(stdout);
...
void send_rip_update(struct sr_instance *sr){
	printf("Acquiring lock for update...\n");
    pthread_mutex_lock(&(sr->rt_lock));
...
void update_route_table(struct sr_instance *sr, sr_ip_hdr_t* ip_packet ,sr_rip_pkt_t* rip_packet, char* iface){
	printf("Acquiring lock for update table...\n");
	pthread_mutex_lock(&(sr->rt_lock));
